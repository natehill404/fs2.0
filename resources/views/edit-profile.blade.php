                            ...
                            <form method="POST" action="{{route('update.profile')}}" enctype="multipart/form-data">
                                ...
                                <div class="form-group row">
                                    <div class="col-md-4 d-flex flex-md-column align-items-md-end">
                                        <label for="image" class="col-form-label text-md-right">{{ __('Image') }} <span
                                                style="font-weight: 200;">(Optional)</span> </label>
                                        <img id="result-image"
                                             src="{{Auth::user()->image ? asset('storage/images/users/'.Auth::user()->image) : asset('img/profile/male.png')}}"
                                             class="ml-md-0 ml-3" width="50" alt="{{Auth::user()->name}}">
                                    </div>
                                    <div class="col-md-6">
                                        <input id="image" type="file" accept="image/*"
                                               class="form-control @error('image') is-invalid @enderror">
                                        <input type="hidden" name="image" id="server-result-image" class="d-none"
                                               hidden>
                                        @error('image')
                                        <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
