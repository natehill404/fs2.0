...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if($fetch->image){
            $avatar = asset('storage/images/users/'.$fetch->image);
        }elseif($fetch->gender === "male"){
            $avatar = asset('img/profile/male.png');
        }elseif($fetch->gender === "female"){
            $avatar = asset('img/profile/female.png');
        }else{
    ...
    public function search(Request $request)
    {
        ...
        foreach ($records->get() as $record) {
            if (Auth::user()->isFriendWith($record)){
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed_images)) {
                    ...
                    if (Auth::user()->image != config('chatify.user_avatar.default')) {
                        $path = storage_path('storage/images/users/'.Auth::user()->image);
                        if (file_exists($path)) {
                            @unlink($path);
                        }
                    }
                    ...
                    $request->avatar->move(public_path('storage/images/users'), $avatar);
                    /*$file->storeAs("images/users/", $avatar);*/
                    $success = $update ? 1 : 0;
                } else {
